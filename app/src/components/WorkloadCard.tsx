      ...
      <ul className="mt-2 space-y-2">
        {weeks.map((w) => (
          <li key={w.start.toISOString()} className="flex items-center gap-3">
            <span className="w-16 shrink-0 text-xs font-medium text-stone-500">
            ...
            <span className="w-24 shrink-0 text-right text-xs text-stone-400">
              {w.pct}% · {w.jobCount} job{w.jobCount === 1 ? "" : "s"}
            </span>
          </li>
        ))}
      </ul>
      <p className="mt-2 text-[11px] text-stone-400">Based on a 5-day work week ({WORKDAY_MINS} min/day).</p>
    </details>
  );
}
