...
export function ReportEditor({
  ...
  return (
    <div className="space-y-4">
      ...
      {rooms.length === 0 ? (
        <p className="text-xs text-stone-400">Add a room above (or use Auto-fill) to build a completion checklist.</p>
      ) : (
        <div className="space-y-3">
          {rooms.map((room, ri) => {
            ...
            return (
              <div key={ri} className="rounded-xl bg-stone-50 p-3 ring-1 ring-inset ring-stone-200">
                <div className="mb-2 flex items-center gap-2">
                  ...
                  />
                  <button type="button" onClick={() => removeRoom(ri)} className="rounded-lg p-1.5 text-stone-400 hover:bg-stone-200" aria-label="Remove room">
                    <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                ...
                {items.length > 0 && (
                  <p className="mb-1.5 text-xs font-medium text-stone-500">
                    ...
                    {outstanding > 0 && <span className="text-amber-600"> · {outstanding} to do</span>}
                  </p>
                )}
                ...
                <div className="space-y-1.5">
                  {items.map((it, ii) => (
                    <div key={ii} className="flex items-center gap-2">
                      ...
                      />
                      <input
                        className={`input flex-1 ${it.done ? "text-stone-400 line-through" : ""}`}
                        value={it.label}
                        placeholder="Checklist item"
                        onChange={(e) => setItem(ri, ii, { label: e.target.value })}
                      />
                      <button type="button" onClick={() => removeItem(ri, ii)} className="rounded-lg p-1 text-stone-400 hover:bg-stone-200" aria-label="Remove item">
                        <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                          <path d="M6 6l12 12M18 6L6 18" strokeLinecap="round" />
                        </svg>
                      </button>
      ...
      <details className="rounded-xl border border-stone-200">
        <summary className="cursor-pointer px-3 py-2.5 text-sm font-semibold text-stone-700">Maintenance details (optional)</summary>
