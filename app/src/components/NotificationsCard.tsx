...
export function NotificationsCard() {
  ...
  return (
    <div className="card mb-4 p-4">
      <h2 className="mb-1 font-semibold text-slate-900 dark:text-slate-100">Notifications</h2>
      <p className="mb-3 text-sm text-slate-500 dark:text-slate-400">Get a push to this device when the inbox check finds new jobs to confirm or flags one for review.</p>

      {state === "loading" && <p className="text-sm text-slate-400 dark:text-slate-500">Checking…</p>}
      {state === "unsupported" && <p className="text-sm text-slate-500 dark:text-slate-400">This device/browser doesn’t support push notifications. On iPhone, add the app to your home screen first.</p>}
      {state === "unconfigured" && (
