...
# Every minute. Idempotent: drop any existing auto-deploy line first.
# (flock in auto-deploy.sh prevents overlap while a build is still running.)
LINE="* * * * * /usr/bin/env bash $SCRIPT"
TMP="$(mktemp)"
...
echo "Auto-deploy installed: checks origin/master every minute and runs update.sh on new commits."
